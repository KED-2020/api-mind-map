...
namespace :queues do
  ...
  task :create => :config do
    ...
    @sqs.create_queue(queue_name: @api.config.SCHEDULED_QUEUE)

    q_url = @sqs.get_queue_url(queue_name: @api.config.SCHEDULED_QUEUE).queue_url
    puts 'Queue created:'
    puts "  Name: #{@api.config.SCHEDULED_QUEUE}"
  ...
  task :purge => :config do
    ...
    puts "Queue #{@api.config.SCHEDULED_QUEUE} purged"
  rescue StandardError => e
